...
const (
	// File extension for all package manifests written by Generator.
...
type Generator struct {
	...
	ChannelName string
	// If true, ChannelName will be the package manifests' default channel.
	IsDefaultChannel bool

	// Func that returns a base PackageManifest.
	getBase getBaseFunc
	// Func that returns the writer the generated PackageManifest's bytes are
	// written to.
...
// WithFileWriter sets a Generator's writer to a package manifest file under
// <dir>.
...
// Generate configures the generator opts then runs it.
...
// validatePackageManifest will validate pkg and log warnings and errors. If
// a validation error is encountered, an error is returned.
